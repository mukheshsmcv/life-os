...
exports.getCurrentTimeStringIST = getCurrentTimeStringIST;
exports.parseNaturalDateString = parseNaturalDateString;
 ...
 * Used when building context for the AI server.
 */
function getCurrentTimeStringIST() {
    const d = nowInIST();
...
function parseNaturalDateString(inputStr, baseDateStr) {
    ...
    if (weekdayTermMatch) {
        ...
        let daysAhead = 0;
        if (prefix === 'next') {
            if (currWeekIndex >= 1 && currWeekIndex <= 4)
                daysAhead = 1;
            else if (currWeekIndex === 5)
                daysAhead = 3;
            else if (currWeekIndex === 6)
                daysAhead = 2;
            else if (currWeekIndex === 7)
                daysAhead = 1;
        }
        else {
            if (currWeekIndex >= 1 && currWeekIndex <= 5)
                daysAhead = 0;
            else if (currWeekIndex === 6)
                daysAhead = 2;
            else if (currWeekIndex === 7)
