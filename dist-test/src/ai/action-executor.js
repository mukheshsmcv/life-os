...
exports.areCreateTasksEquivalent = areCreateTasksEquivalent;
exports.deduplicateActions = deduplicateActions;
...
function deduplicateActions(actions) {
    ...
    for (const action of actions) {
        if (action.type === 'create_task') {
            const isDuplicate = seenCreateTasks.some((seen) => areCreateTasksEquivalent(seen, action.payload));
            if (isDuplicate) {
                continue;
            }
...
function findNextScheduledTitle(tasks, settings, currentTime) {
    const schedule = (0, scheduler_1.scheduleTasks)(tasks, settings, currentTime);
...
function executeAction(action, context) {
    ...
    const settings = context.settings ?? scheduler_1.DEFAULT_SCHEDULING_SETTINGS;
    switch (action.type) {
        case 'process_intent': {
            ...
            if (operation === 'create_activity') {
                if (!title)
                    return { success: false, message: 'Missing title for activity creation.' };
                ...
                if (mode === 'fixed') {
                    if (context.operations.addEvent) {
                        createdId = context.operations.addEvent({
                            ...
                            date: scheduling?.date ?? (0, date_time_1.getTodayString)(),
                            startMinute: scheduling?.startMinute ?? 0,
                            endMinute: scheduling?.endMinute ?? (scheduling?.startMinute ?? 0) + (duration ?? 60),
                        });
                        const dateLabel = scheduling?.date && scheduling.date !== (0, date_time_1.getTodayString)() ? ` on ${(0, date_time_1.formatDisplayDate)(scheduling.date)}` : '';
                        return {
                            success: true,
                ...
                const dateLabel = taskDate && taskDate !== (0, date_time_1.getTodayString)() ? `\nScheduled for ${(0, date_time_1.formatDisplayDate)(taskDate)}.` : '';
                return {
                    success: true,
            ...
            if (operation === 'log_constraint') {
                if (!title)
                    return { success: false, message: 'Missing title for constraint.' };
            ...
            if (operation === 'update_activity') {
                const resolvedTaskId = action.payload.targetId || action.payload.taskId;
                if (!resolvedTaskId) {
                ...
                return executeAction({ type: 'update_task', payload: { ...action.payload, taskId: resolvedTaskId } }, context);
            }
            ...
            if (operation === 'complete_activity') {
                ...
                const updatedTasks = context.tasks.map(t => t.id === targetId ? { ...t, status: 'completed' } : t);
                const nextTitle = findNextScheduledTitle(updatedTasks, settings, currentTime);
            ...
            if (operation === 'skip_activity') {
                ...
                const updatedTasks = context.tasks.map(t => t.id === targetId ? { ...t, status: 'skipped' } : t);
                const nextTitle = findNextScheduledTitle(updatedTasks, settings, currentTime);
                const nextMsg = nextTitle ? `\n${nextTitle} is now next.` : '\nNo remaining activities scheduled today.';
        ...
        case 'create_task': {
            ...
            if (context.createdTasksHistory) {
                const isDuplicate = context.createdTasksHistory.some((seen) => areCreateTasksEquivalent(seen, action.payload));
                if (isDuplicate) {
                    return {
                        success: true,
            ...
            const dateLabel = taskDate && taskDate !== todayStr
                ? `\nScheduled for ${(0, date_time_1.formatDisplayDate)(taskDate)}.`
                : '';
            // "Next up" is computed from today's visible tasks:
            // explicitly today-dated tasks  +  undated tasks (which are always shown on Today).
            const todayTasks = updatedTasks.filter((t) => t.date === todayStr || t.date == null);
            const nextTitle = !taskDate || taskDate === todayStr
        ...
        case 'complete_task': {
            ...
            const updatedTasks = context.tasks.map((t) => t.id === taskId ? { ...t, status: 'completed' } : t);
            const nextTitle = findNextScheduledTitle(updatedTasks, settings, currentTime);
            const nextMsg = nextTitle ? `\n${nextTitle} is now next.` : '\nNo remaining activities scheduled today.';
            return {
                success: true,
        ...
        case 'skip_task': {
            ...
            const updatedTasks = context.tasks.map((t) => t.id === taskId ? { ...t, status: 'skipped' } : t);
            const nextTitle = findNextScheduledTitle(updatedTasks, settings, currentTime);
            const nextMsg = nextTitle ? `\n${nextTitle} is now next.` : '\nNo remaining activities scheduled today.';
            return {
                success: true,
        ...
        case 'update_task': {
            ...
            if (date !== undefined)
                updates.date = date;
            if (scheduledStartMinute !== undefined)
                updates.scheduledStartMinute = scheduledStartMinute;
            if (scheduling !== undefined) {
                updates.scheduling = { ...targetTask.scheduling, ...scheduling };
                if (scheduling.date !== undefined)
                    updates.date = scheduling.date;
                if (scheduling.startMinute !== undefined)
                    updates.scheduledStartMinute = scheduling.startMinute;
                if (scheduling.durationMinutes !== undefined)
                    updates.durationMinutes = scheduling.durationMinutes ?? undefined;
            }
            ...
            if (updates.title !== undefined)
                details.push(`title: "${updates.title}"`);
            if (updates.durationMinutes !== undefined)
                details.push(`duration: ${updates.durationMinutes}m`);
            if (updates.priority !== undefined)
                details.push(`priority: ${updates.priority}`);
            if (updates.date !== undefined)
                details.push(updates.date === null ? 'date: anytime (undated)' : `date: ${(0, date_time_1.formatDisplayDate)(updates.date)}`);
            if (updates.entities?.people)
                details.push(`people: ${updates.entities.people.join(', ')}`);
        ...
        case 'get_schedule': {
            ...
            const dateLabel = (0, date_time_1.formatDisplayDate)(targetDateStr);
            ...
            if (targetDateStr === todayStr) {
                ...
                refDate = (0, date_time_1.parseDateString)(targetDateStr);
            }
            const schedule = (0, scheduler_1.scheduleTasks)(tasksForDate, settings, refDate);
            const taskMap = new Map(context.tasks.map((t) => [t.id, t]));
            if (action.payload?.scope === 'next') {
                let nextBlock = null;
                const inProgressIndex = schedule.blocks.findIndex((b) => currentTime >= b.start && currentTime < b.end);
                ...
                if (!nextBlock) {
                    const noActivityMsg = targetDateStr === todayStr
        ...
        case 'replan_day': {
            const schedule = (0, scheduler_1.scheduleTasks)(context.tasks, settings, currentTime);
        ...
        case 'get_free_time': {
            ...
            const schedule = (0, scheduler_1.scheduleTasks)(tasksForDate, settings, refDate);
            const scheduledMinutes = schedule.blocks.reduce((acc, b) => acc + (b.endMinute - b.startMinute), 0);
...
function executeUpdateEvent(payload, context) {
    ...
    if (endMinute !== undefined)
        updates.endMinute = endMinute;
    if (notes !== undefined)
        updates.notes = notes;
    if (scheduling !== undefined) {
        updates.scheduling = { ...targetEvent.scheduling, ...scheduling };
        if (scheduling.date !== undefined)
            updates.date = scheduling.date ?? undefined;
        if (scheduling.startMinute !== undefined)
            updates.startMinute = scheduling.startMinute ?? undefined;
        if (scheduling.endMinute !== undefined)
            updates.endMinute = scheduling.endMinute ?? undefined;
    }
    if (entities !== undefined) {
    ...
    if (updates.title !== undefined)
        details.push(`title: "${updates.title}"`);
    if (updates.date !== undefined)
        details.push(updates.date === undefined ? 'date: anytime' : `date: ${updates.date}`);
    if (updates.entities?.people)
        details.push(`people: ${updates.entities.people.join(', ')}`);
    const changesMsg = details.length > 0 ? ` (${details.join(', ')})` : '';
    return {
