    ...
    switch (action.type) {
        case 'process_intent': {
            ...
            if (['create_activity', 'update_activity'].includes(payload.operation)) {
                ...
                if (payload.scheduling?.date) {
                    if (!(0, date_time_1.isValidDateString)(payload.scheduling.date)) {
                        return {
                            valid: false,
            ...
            if (payload.operation === 'update_activity' || payload.operation === 'delete_activity' || payload.operation === 'complete_activity' || payload.operation === 'skip_activity') {
                ...
                if (!payload.targetId && targetQuery) {
                    const matchResult = (0, mock_intent_parser_1.resolveTaskEntity)(targetQuery, tasks);
        ...
        case 'create_task': {
            ...
            if (action.payload.date !== undefined) {
                if (!(0, date_time_1.isValidDateString)(action.payload.date)) {
                    return {
                        valid: false,
        ...
        case 'update_task': {
            if (action.type === 'update_task') {
                ...
                if (date !== undefined && date !== null && !(0, date_time_1.isValidDateString)(date)) {
                    return {
                        valid: false,
            ...
            const matchResult = (0, mock_intent_parser_1.resolveTaskEntity)(taskTitleQuery, tasks);
            if (matchResult.type === 'none') {
                return {
                    valid: false,
        ...
        case 'get_free_time': {
            if (action.type === 'get_schedule' || action.type === 'get_free_time') {
                ...
                if (d !== undefined && d !== null && !(0, date_time_1.isValidDateString)(d)) {
                    return {
                        valid: false,
